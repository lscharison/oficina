...
export default React.memo(() => {
  ...
  const {zoomStatus, pagePath} = useSelector((state: TStore) => state.sport.display);
  ...
  return matchDetail ? (
    <div className={styles.wrapper} ref={container}>
      ...
      {zoomStatus ? <Statistician/> : <>
            ...
            {!matchDetail.matchClock.period ? (
              ...
              <div>
                ...
                </span>
                {
                  ...
                  <div className='text-num'>{`${matchDetail.score.home} : ${matchDetail.score.away}`}</div>
                }
              </div>
            )}
          </div>
        ...
        <div className={classnames('play-list', {'is-empty': !playList.length, 'is-esports': pagePath === '/esports'})} ref={playListRef} >
                              ...
                              (market) =>
                                market.ops.length && (
                                  <div className='market-item' key={market.mkId}>
                                    <div className='label text-theme text-num'>{market.ops[0].name.split(' ')[1]}</div>
                                    {market.ops.map((bil) => (
                                      <div
                                        ...
                                        key={bil.id}
                                        onClick={() => addAndRemoveOrder({...bil, sportId: matchDetail.sportId})}>
                                        {bil.locked ? (
                                          <LockOutlined />
                                        ) : bil.available ? (
                      ...
                      <div className='play-content'>
                        <div className='play-bd play-default'>
                          ...
                          <div className='bd-list'>
                            {getBoDanList(item.mks[0].ops).map((bil, bidx) =>
                              bil ? (
                                <OddItem bil={{...bil, sportId: matchDetail.sportId}} kc={item.code} key={bil?.id || bidx} />
                              ) : (
                    ...
                    return (
                      <div className='paly-content'>
                        <div className='play-default'>
                          {item.mks.map((market) => (
                            <div className={classnames('market-item', {'one-line': item.code.includes('_1X2') || item.code.includes('wdl_')})} key={market.mkId}>
